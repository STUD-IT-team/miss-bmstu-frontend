    ...
    <section class="section-text">
      <ViewPortWatcher on-ref="nomination-1" v-model="nominationsVisible[1]" :bottom-offset="100" :top-offset="-200">
        <div ref="nomination-1" class="nomination-container" :class="{'hidden': !nominationsVisible[1]}">
          ...
          <p>В финале все оценки складываются. Участница с самым высоким суммарным баллом становится победительницей конкурса, а <mark>второе место</mark> занимает та, кто набрала следующий по величине результат — ей присуждается титул <mark>«Вице-мисс»</mark>.</p>
        </div>
      </ViewPortWatcher>
      ...
      <ViewPortWatcher on-ref="nomination-2" v-model="nominationsVisible[2]" :bottom-offset="100" :top-offset="-200">
        <div ref="nomination-2" class="nomination-container" :class="{'hidden': !nominationsVisible[2]}">
          ...
          <p>В этой номинации все <mark>решают зрители финала конкурса</mark>. Каждый, кто пришел поддержать участниц, получает возможность проголосовать за ту, кто, по их мнению, заслуживает победы.</p>
        </div>
      </ViewPortWatcher>
