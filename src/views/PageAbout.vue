    ...
    <section class="section-text">
      <ViewPortWatcher on-ref="nomination-1" v-model="nominationsVisible[1]" :bottom-offset="100" :top-offset="-200">
        <div ref="nomination-1" class="nomination-container" :class="{'hidden': !nominationsVisible[1]}">
          ...
          <p>Главная номинация присуждается победительнице конкурса – участнице, набравшей наибольшее количество баллов по итогам всех пяти туров финала.</p>
          ...
          <p>В финале все оценки складываются. Участница с самым высоким суммарным баллом становится победительницей конкурса, а второе место занимает та, кто набрала следующий по величине результат — ей присуждается титул «Вице-мисс».</p>
        </div>
      </ViewPortWatcher>
      ...
      <ViewPortWatcher on-ref="nomination-2" v-model="nominationsVisible[2]" :bottom-offset="100" :top-offset="-200">
        <div ref="nomination-2" class="nomination-container" :class="{'hidden': !nominationsVisible[2]}">
          ...
          <p>В этой номинации все решают зрители финала конкурса. Каждый, кто пришел поддержать участниц, получает возможность проголосовать за ту, кто, по их мнению, заслуживает победы.</p>
        </div>
      </ViewPortWatcher>
      ...
      <ViewPortWatcher on-ref="nomination-3" v-model="nominationsVisible[3]" :bottom-offset="100" :top-offset="-200">
        <div ref="nomination-3" class="nomination-container" :class="{'hidden': !nominationsVisible[3]}">
          ...
          <p>Победительница этой номинации определяется SMS-голосованием, которое проходит в период подготовки к конкурсу и останавливается прямо на финале. Результаты обновляются в режиме реального времени на сайте, а самый активный участник голосования — тот, кто отправит больше всего SMS, — получит шанс выйти на сцену во время финала и лично вручить приз победительнице!</p>
        </div>
      </ViewPortWatcher>
      ...
      <ViewPortWatcher on-ref="nomination-4" v-model="nominationsVisible[4]" :bottom-offset="100" :top-offset="-200">
        <div ref="nomination-4" class="nomination-container" :class="{'hidden': !nominationsVisible[4]}">
          ...
          <p>Единственная номинация, где победительницу выбирают сами участницы конкурса. Тайное голосование проходит за день до финала — девушки отдают голос той, кто стала душой коллектива и поддержкой для всех во время подготовки.</p>
        </div>
      </ViewPortWatcher>
      ...
      <ViewPortWatcher on-ref="nomination-5" v-model="nominationsVisible[5]" :bottom-offset="100" :top-offset="-200">
        <div ref="nomination-5" class="nomination-container" :class="{'hidden': !nominationsVisible[5]}">
          ...
          <p>Номинация, где победительницу выбирает оргкомитет конкурса. В процессе подготовки с финалистками работают хореографы, постановщик дефиле, режиссёры, стилисты, фотографы. Оргкомитет оценивает, насколько участницы организованы, собраны и мотивированы, а затем с помощью тайного голосования выбирает ту, которая приложила максимум усилий на пути к финалу.</p>
        </div>
      </ViewPortWatcher>
