import ImageMiss1Jpg200 from "#~/images/miss-photos/avatar-photos/jpg-200/1.jpg";
import ImageMiss2Jpg200 from "#~/images/miss-photos/avatar-photos/jpg-200/2.jpg";
import ImageMiss3Jpg200 from "#~/images/miss-photos/avatar-photos/jpg-200/3.jpg";
import ImageMiss4Jpg200 from "#~/images/miss-photos/avatar-photos/jpg-200/4.jpg";
import ImageMiss5Jpg200 from "#~/images/miss-photos/avatar-photos/jpg-200/5.jpg";
import ImageMiss6Jpg200 from "#~/images/miss-photos/avatar-photos/jpg-200/6.jpg";
import ImageMiss7Jpg200 from "#~/images/miss-photos/avatar-photos/jpg-200/7.jpg";
import ImageMiss8Jpg200 from "#~/images/miss-photos/avatar-photos/jpg-200/8.jpg";
import ImageMiss9Jpg200 from "#~/images/miss-photos/avatar-photos/jpg-200/9.jpg";
// import ImageMiss10Jpg200 from "#~/images/miss-photos/avatar-photos/jpg-200/10.jpg";
import ImageMiss1Jpg400 from "#~/images/miss-photos/avatar-photos/jpg-400/1.jpg";
import ImageMiss2Jpg400 from "#~/images/miss-photos/avatar-photos/jpg-400/2.jpg";
import ImageMiss3Jpg400 from "#~/images/miss-photos/avatar-photos/jpg-400/3.jpg";
import ImageMiss4Jpg400 from "#~/images/miss-photos/avatar-photos/jpg-400/4.jpg";
import ImageMiss5Jpg400 from "#~/images/miss-photos/avatar-photos/jpg-400/5.jpg";
import ImageMiss6Jpg400 from "#~/images/miss-photos/avatar-photos/jpg-400/6.jpg";
import ImageMiss7Jpg400 from "#~/images/miss-photos/avatar-photos/jpg-400/7.jpg";
import ImageMiss8Jpg400 from "#~/images/miss-photos/avatar-photos/jpg-400/8.jpg";
import ImageMiss9Jpg400 from "#~/images/miss-photos/avatar-photos/jpg-400/9.jpg";
// import ImageMiss10Jpg400 from "#~/images/miss-photos/avatar-photos/jpg-400/10.jpg";
import ImageMiss1Jpg800 from "#~/images/miss-photos/avatar-photos/jpg-800/1.jpg";
import ImageMiss2Jpg800 from "#~/images/miss-photos/avatar-photos/jpg-800/2.jpg";
import ImageMiss3Jpg800 from "#~/images/miss-photos/avatar-photos/jpg-800/3.jpg";
import ImageMiss4Jpg800 from "#~/images/miss-photos/avatar-photos/jpg-800/4.jpg";
import ImageMiss5Jpg800 from "#~/images/miss-photos/avatar-photos/jpg-800/5.jpg";
import ImageMiss6Jpg800 from "#~/images/miss-photos/avatar-photos/jpg-800/6.jpg";
import ImageMiss7Jpg800 from "#~/images/miss-photos/avatar-photos/jpg-800/7.jpg";
import ImageMiss8Jpg800 from "#~/images/miss-photos/avatar-photos/jpg-800/8.jpg";
import ImageMiss9Jpg800 from "#~/images/miss-photos/avatar-photos/jpg-800/9.jpg";
// import ImageMiss10Jpg800 from "#~/images/miss-photos/avatar-photos/jpg-800/10.jpg";

import ImageMiss1Webp200 from "#~/images/miss-photos/avatar-photos/webp-200/1.webp";
import ImageMiss2Webp200 from "#~/images/miss-photos/avatar-photos/webp-200/2.webp";
import ImageMiss3Webp200 from "#~/images/miss-photos/avatar-photos/webp-200/3.webp";
import ImageMiss4Webp200 from "#~/images/miss-photos/avatar-photos/webp-200/4.webp";
import ImageMiss5Webp200 from "#~/images/miss-photos/avatar-photos/webp-200/5.webp";
import ImageMiss6Webp200 from "#~/images/miss-photos/avatar-photos/webp-200/6.webp";
import ImageMiss7Webp200 from "#~/images/miss-photos/avatar-photos/webp-200/7.webp";
import ImageMiss8Webp200 from "#~/images/miss-photos/avatar-photos/webp-200/8.webp";
import ImageMiss9Webp200 from "#~/images/miss-photos/avatar-photos/webp-200/9.webp";
// import ImageMiss10Webp200 from "#~/images/miss-photos/avatar-photos/webp-200/10.webp";
import ImageMiss1Webp400 from "#~/images/miss-photos/avatar-photos/webp-400/1.webp";
import ImageMiss2Webp400 from "#~/images/miss-photos/avatar-photos/webp-400/2.webp";
import ImageMiss3Webp400 from "#~/images/miss-photos/avatar-photos/webp-400/3.webp";
import ImageMiss4Webp400 from "#~/images/miss-photos/avatar-photos/webp-400/4.webp";
import ImageMiss5Webp400 from "#~/images/miss-photos/avatar-photos/webp-400/5.webp";
import ImageMiss6Webp400 from "#~/images/miss-photos/avatar-photos/webp-400/6.webp";
import ImageMiss7Webp400 from "#~/images/miss-photos/avatar-photos/webp-400/7.webp";
import ImageMiss8Webp400 from "#~/images/miss-photos/avatar-photos/webp-400/8.webp";
import ImageMiss9Webp400 from "#~/images/miss-photos/avatar-photos/webp-400/9.webp";
// import ImageMiss10Webp400 from "#~/images/miss-photos/avatar-photos/webp-400/10.webp";
import ImageMiss1Webp800 from "#~/images/miss-photos/avatar-photos/webp-800/1.webp";
import ImageMiss2Webp800 from "#~/images/miss-photos/avatar-photos/webp-800/2.webp";
import ImageMiss3Webp800 from "#~/images/miss-photos/avatar-photos/webp-800/3.webp";
import ImageMiss4Webp800 from "#~/images/miss-photos/avatar-photos/webp-800/4.webp";
import ImageMiss5Webp800 from "#~/images/miss-photos/avatar-photos/webp-800/5.webp";
import ImageMiss6Webp800 from "#~/images/miss-photos/avatar-photos/webp-800/6.webp";
import ImageMiss7Webp800 from "#~/images/miss-photos/avatar-photos/webp-800/7.webp";
import ImageMiss8Webp800 from "#~/images/miss-photos/avatar-photos/webp-800/8.webp";
import ImageMiss9Webp800 from "#~/images/miss-photos/avatar-photos/webp-800/9.webp";
// import ImageMiss10Webp800 from "#~/images/miss-photos/avatar-photos/webp-800/10.webp";


import imageMiss1Profile1Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/1/1.jpg"
import imageMiss1Profile2Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/1/2.jpg"
import imageMiss1Profile3Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/1/3.jpg"
import imageMiss1Profile4Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/1/4.jpg"
import imageMiss1Profile5Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/1/5.jpg"
import imageMiss2Profile1Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/2/1.jpg"
import imageMiss2Profile2Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/2/2.jpg"
import imageMiss2Profile3Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/2/3.jpg"
import imageMiss2Profile4Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/2/4.jpg"
import imageMiss2Profile5Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/2/5.jpg"
import imageMiss2Profile6Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/2/6.jpg"
import imageMiss3Profile1Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/3/1.jpg"
import imageMiss3Profile2Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/3/2.jpg"
import imageMiss3Profile3Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/3/3.jpg"
import imageMiss3Profile4Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/3/4.jpg"
import imageMiss3Profile5Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/3/5.jpg"
import imageMiss3Profile6Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/3/6.jpg"
import imageMiss4Profile1Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/4/1.jpg"
import imageMiss4Profile2Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/4/2.jpg"
import imageMiss4Profile3Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/4/3.jpg"
import imageMiss4Profile4Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/4/4.jpg"
import imageMiss5Profile1Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/5/1.jpg"
import imageMiss5Profile2Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/5/2.jpg"
import imageMiss5Profile3Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/5/3.jpg"
import imageMiss5Profile4Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/5/4.jpg"
import imageMiss5Profile5Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/5/5.jpg"
import imageMiss6Profile1Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/6/1.jpg"
import imageMiss6Profile2Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/6/2.jpg"
import imageMiss6Profile3Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/6/3.jpg"
import imageMiss6Profile4Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/6/4.jpg"
import imageMiss7Profile1Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/7/1.jpg"
import imageMiss7Profile2Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/7/2.jpg"
import imageMiss7Profile3Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/7/3.jpg"
import imageMiss7Profile4Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/7/4.jpg"
import imageMiss8Profile1Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/8/1.jpg"
import imageMiss8Profile2Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/8/2.jpg"
import imageMiss8Profile3Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/8/3.jpg"
import imageMiss8Profile4Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/8/4.jpg"
import imageMiss9Profile1Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/9/1.jpg"
import imageMiss9Profile2Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/9/2.jpg"
import imageMiss9Profile3Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/9/3.jpg"
import imageMiss9Profile4Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/9/4.jpg"
import imageMiss10Profile1Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/10/1.jpg"
import imageMiss10Profile2Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/10/2.jpg"
import imageMiss10Profile3Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/10/3.jpg"
import imageMiss10Profile4Jpg600 from "#~/images/miss-photos/profile-photos/jpg-600/10/4.jpg"
import imageMiss1Profile1Webp600 from "#~/images/miss-photos/profile-photos/webp-600/1/1.webp"
import imageMiss1Profile2Webp600 from "#~/images/miss-photos/profile-photos/webp-600/1/2.webp"
import imageMiss1Profile3Webp600 from "#~/images/miss-photos/profile-photos/webp-600/1/3.webp"
import imageMiss1Profile4Webp600 from "#~/images/miss-photos/profile-photos/webp-600/1/4.webp"
import imageMiss1Profile5Webp600 from "#~/images/miss-photos/profile-photos/webp-600/1/5.webp"
import imageMiss2Profile1Webp600 from "#~/images/miss-photos/profile-photos/webp-600/2/1.webp"
import imageMiss2Profile2Webp600 from "#~/images/miss-photos/profile-photos/webp-600/2/2.webp"
import imageMiss2Profile3Webp600 from "#~/images/miss-photos/profile-photos/webp-600/2/3.webp"
import imageMiss2Profile4Webp600 from "#~/images/miss-photos/profile-photos/webp-600/2/4.webp"
import imageMiss2Profile5Webp600 from "#~/images/miss-photos/profile-photos/webp-600/2/5.webp"
import imageMiss2Profile6Webp600 from "#~/images/miss-photos/profile-photos/webp-600/2/6.webp"
import imageMiss3Profile1Webp600 from "#~/images/miss-photos/profile-photos/webp-600/3/1.webp"
import imageMiss3Profile2Webp600 from "#~/images/miss-photos/profile-photos/webp-600/3/2.webp"
import imageMiss3Profile3Webp600 from "#~/images/miss-photos/profile-photos/webp-600/3/3.webp"
import imageMiss3Profile4Webp600 from "#~/images/miss-photos/profile-photos/webp-600/3/4.webp"
import imageMiss3Profile5Webp600 from "#~/images/miss-photos/profile-photos/webp-600/3/5.webp"
import imageMiss3Profile6Webp600 from "#~/images/miss-photos/profile-photos/webp-600/3/6.webp"
import imageMiss4Profile1Webp600 from "#~/images/miss-photos/profile-photos/webp-600/4/1.webp"
import imageMiss4Profile2Webp600 from "#~/images/miss-photos/profile-photos/webp-600/4/2.webp"
import imageMiss4Profile3Webp600 from "#~/images/miss-photos/profile-photos/webp-600/4/3.webp"
import imageMiss4Profile4Webp600 from "#~/images/miss-photos/profile-photos/webp-600/4/4.webp"
import imageMiss5Profile1Webp600 from "#~/images/miss-photos/profile-photos/webp-600/5/1.webp"
import imageMiss5Profile2Webp600 from "#~/images/miss-photos/profile-photos/webp-600/5/2.webp"
import imageMiss5Profile3Webp600 from "#~/images/miss-photos/profile-photos/webp-600/5/3.webp"
import imageMiss5Profile4Webp600 from "#~/images/miss-photos/profile-photos/webp-600/5/4.webp"
import imageMiss5Profile5Webp600 from "#~/images/miss-photos/profile-photos/webp-600/5/5.webp"
import imageMiss6Profile1Webp600 from "#~/images/miss-photos/profile-photos/webp-600/6/1.webp"
import imageMiss6Profile2Webp600 from "#~/images/miss-photos/profile-photos/webp-600/6/2.webp"
import imageMiss6Profile3Webp600 from "#~/images/miss-photos/profile-photos/webp-600/6/3.webp"
import imageMiss6Profile4Webp600 from "#~/images/miss-photos/profile-photos/webp-600/6/4.webp"
import imageMiss7Profile1Webp600 from "#~/images/miss-photos/profile-photos/webp-600/7/1.webp"
import imageMiss7Profile2Webp600 from "#~/images/miss-photos/profile-photos/webp-600/7/2.webp"
import imageMiss7Profile3Webp600 from "#~/images/miss-photos/profile-photos/webp-600/7/3.webp"
import imageMiss7Profile4Webp600 from "#~/images/miss-photos/profile-photos/webp-600/7/4.webp"
import imageMiss8Profile1Webp600 from "#~/images/miss-photos/profile-photos/webp-600/8/1.webp"
import imageMiss8Profile2Webp600 from "#~/images/miss-photos/profile-photos/webp-600/8/2.webp"
import imageMiss8Profile3Webp600 from "#~/images/miss-photos/profile-photos/webp-600/8/3.webp"
import imageMiss8Profile4Webp600 from "#~/images/miss-photos/profile-photos/webp-600/8/4.webp"
import imageMiss9Profile1Webp600 from "#~/images/miss-photos/profile-photos/webp-600/9/1.webp"
import imageMiss9Profile2Webp600 from "#~/images/miss-photos/profile-photos/webp-600/9/2.webp"
import imageMiss9Profile3Webp600 from "#~/images/miss-photos/profile-photos/webp-600/9/3.webp"
import imageMiss9Profile4Webp600 from "#~/images/miss-photos/profile-photos/webp-600/9/4.webp"
import imageMiss10Profile1Webp600 from "#~/images/miss-photos/profile-photos/webp-600/10/1.webp"
import imageMiss10Profile2Webp600 from "#~/images/miss-photos/profile-photos/webp-600/10/2.webp"
import imageMiss10Profile3Webp600 from "#~/images/miss-photos/profile-photos/webp-600/10/3.webp"
import imageMiss10Profile4Webp600 from "#~/images/miss-photos/profile-photos/webp-600/10/4.webp"

export const missList = [
  {
    id: 1, name: 'Густова Анастасия', group: 'ИБМ2-41Б',
    imageWebp: `${ImageMiss1Webp200} 200w, ${ImageMiss1Webp400} 400w,  ${ImageMiss1Webp800} 800w`,
    imageJpg: `${ImageMiss1Jpg200} 200w, ${ImageMiss1Jpg400} 400w,  ${ImageMiss1Jpg800} 800w`,
    bioImages: [],
    QAtexts: [
      {
        question: '',
        answer: 'Воздух — это вечное движение, это жажда простора и невозможность стоять на месте. Моя суть — это стремление. Стремление наполнить каждый свой вдох чем-то ярким, незабываемым, таким, от чего захватывает дух. Я не жду у моря погоды — я сама создаю ветер перемен. Для меня чувствовать себя живой — значит быть в ритме. В ритме города, танца, новых открытий. Я словно разгоняюсь до той самой скорости, где прошлое остается красивым кадром в зеркале заднего вида, а будущее манит своей бесконечностью.\n\n«Очарование — искренность и принятие себя до безумства, даже самая глупая шутка притягательна и вызывает улыбку. Очаровательные люди не только про красоту, а про внутреннее ощущение этой жизни, про умение радоваться мелочам»\n<i>Густова Анастасия</i>',
      },
    ]
  },
  {
    id: 2, name: 'Мухамедшина Виолетта', group: 'СГН2-61Б',
    imageWebp: `${ImageMiss2Webp200} 200w, ${ImageMiss2Webp400} 400w,  ${ImageMiss2Webp800} 800w`,
    imageJpg: `${ImageMiss2Jpg200} 200w, ${ImageMiss2Jpg400} 400w,  ${ImageMiss2Jpg800} 800w`,
    bioImages: [],
    QAtexts: [
      {
        question: '',
        answer: 'Моя стихия — вода. Мне нравится ее глубина, сила и таинственность. Мы можем видеть спокойную синюю гладь, но не знаем, сколько подводных течений таится под ней. А возможно, легкие волны — это лишь затишье перед бурей, и скоро эта стихия накроет с головой.\n\n«Очарование — это сочетание природной харизмы, нежности и внутренней красоты, способность дарить радость всем людям вокруг»\n<i>Мухамедшина Виолетта</i>',
      },
    ]
  },
  {
    id: 3, name: 'Глухих Полина', group: 'МТ9-41Б',
    imageWebp: `${ImageMiss3Webp200} 200w, ${ImageMiss3Webp400} 400w,  ${ImageMiss3Webp800} 800w`,
    imageJpg: `${ImageMiss3Jpg200} 200w, ${ImageMiss3Jpg400} 400w,  ${ImageMiss3Jpg800} 800w`,
    bioImages: [],
    QAtexts: [
      {
        question: '',
        answer: 'Моя стихия — вода. Она может быть и спокойной, и бушевать в шторме, но при этом остается плавной и масштабной, что успокаивает душу, и одновременно завораживает.\n\n«Очарование — состояние души, когда человек умеет любить жизнь, людей и самого себя. Это искренность и способность видеть красоту в мире и делиться этой красотой!»\n<i>Глухих Полина</i>',
      },
    ]
  },
  {
    id: 4, name: 'Васина Софья', group: 'ЮР-41',
    imageWebp: `${ImageMiss4Webp200} 200w, ${ImageMiss4Webp400} 400w,  ${ImageMiss4Webp800} 800w`,
    imageJpg: `${ImageMiss4Jpg200} 200w, ${ImageMiss4Jpg400} 400w,  ${ImageMiss4Jpg800} 800w`,
    bioImages: [],
    QAtexts: [
      {
        question: '',
        answer: 'Моя стихия — земля. Незыблемый фундамент и монолитная опора, на которой строится порядок среди хаоса. Она проявляется в моей гранитной стойкости перед любыми задачами и в глубоких пластах знаний, дающих вам твердую почву под ногами. Я — та самая благодатная твердь, где ваши идеи пускают крепкие корни и превращаются в великие свершения.\n\n«Очарование — это сочетание внутренней уверенности, доброты и харизмы. Это способность притягивать людей к себе и оставлять о себе положительное впечатление»\n<i>Васина Софья</i>',
      },
    ]
  },
  {
    id: 5, name: 'Усова Анастасия', group: 'РЛ1-49',
    imageWebp: `${ImageMiss5Webp200} 200w, ${ImageMiss5Webp400} 400w,  ${ImageMiss5Webp800} 800w`,
    imageJpg: `${ImageMiss5Jpg200} 200w, ${ImageMiss5Jpg400} 400w,  ${ImageMiss5Jpg800} 800w`,
    bioImages: [],
    QAtexts: [
      {
        question: '',
        answer: 'Огонь даёт энергию для всего: гореть на сцене, выкладываться до последнего и не бояться быть яркой. Я Овен — первый и загорающийся идеями с пол-оборота. Здесь огонь — не разрушительный, а тёплый и зажигательный, который согревает близких и помогает зажигать других.\n\n«Когда человек не пытается понравиться, но от него невозможно оторвать взгляд. Это про внутреннее состояние человека.»\n<i>Усова Анастасия</i>',
      },
    ]
  },
  {
    id: 6, name: 'Кирей Полина', group: 'СМ10-41',
    imageWebp: `${ImageMiss6Webp200} 200w, ${ImageMiss6Webp400} 400w,  ${ImageMiss6Webp800} 800w`,
    imageJpg: `${ImageMiss6Jpg200} 200w, ${ImageMiss6Jpg400} 400w,  ${ImageMiss6Jpg800} 800w`,
    bioImages: [],
    QAtexts: [
      {
        question: '',
        answer: 'Моя стихия — воздух. Воздушные люди очень общительны и коммуникабельны, это всё про меня. Я люблю нестандартные решения и мне нравится узнавать что-то новое. Я думаю, что смогу раскрыть себя в таком ключе, о котором сама и не догадывалась.\n\n«Очарование — это когда от человека исходит какой-то особенный, тёплый свет. Когда его внутренняя гармония и неповторимость отражается во взгляде, в улыбке, в каждом жесте. Когда человек не притворяется, он настоящий, и эта уникальность завораживает и притягивает, как магия»\n<i>Кирей Полина</i>',
      },
    ]
  },
  {
    id: 7, name: 'Данилова Анастасия', group: 'АК1-101',
    imageWebp: `${ImageMiss7Webp200} 200w, ${ImageMiss7Webp400} 400w,  ${ImageMiss7Webp800} 800w`,
    imageJpg: `${ImageMiss7Jpg200} 200w, ${ImageMiss7Jpg400} 400w,  ${ImageMiss7Jpg800} 800w`,
    bioImages: [],
    QAtexts: [
      {
        question: '',
        answer: 'Почему воздух? Потому что это про меня. Воздух легкий, прозрачный, он не напрягается, чтобы быть везде. Он просто есть. И я такая же: не люблю усложнять, пытаюсь все в этой жизни упростить и просто жить, наслаждаясь. Мне легко с людьми, мне легко в новой компании, мне легко браться за новое дело. Воздух не застрянет в одной точке — ему нужно движение. И мне тоже. Если я перестаю двигаться, пробовать новое, я задыхаюсь. Воздух всегда ищет новое пространство, так же и я. Мне говорят: «Выбери что-то одно: музыка, спорт, шитье», а я не могу. Потому что я как воздух. Я хочу быть везде, пробовать всё, касаться всего.\n\n«Очарование — это когда ты можешь зарядить людей своей улыбкой, поднять им настроение одним взглядом.»\n<i>Данилова Анастасия</i>',
      },
    ]
  },
  {
    id: 8, name: 'Козлова Полина', group: 'ФН11-22Б',
    imageWebp: `${ImageMiss8Webp200} 200w, ${ImageMiss8Webp400} 400w,  ${ImageMiss8Webp800} 800w`,
    imageJpg: `${ImageMiss8Jpg200} 200w, ${ImageMiss8Jpg400} 400w,  ${ImageMiss8Jpg800} 800w`,
    bioImages: [],
    QAtexts: [
      {
        question: '',
        answer: 'Моя стихия — земля. На мой взгляд, это самая разносторонняя стихия: земля может быть и в виде плодородного чернозема, и в виде песка в пустыне, и в виде многолетней мерзлоты. Она может быть разной, но остается стабильной, умеет как получать ресурсы, так и отдавать их безвозмездно, и думаю, именно в этом она отражается во мне.\n\n«Очарование — это совокупность множества положительных качеств: доброты, отзывчивости, искренности, женственности, стойкости, открытости, уверенности и целеустремленности. Это способность быть одновременно и вдохновенной, и вдохновляющей.»\n<i>Козлова Полина</i>',
      },
    ]
  },
  {
    id: 9, name: 'Денисова Ольга', group: 'ИУ10-26',
    imageWebp: `${ImageMiss9Webp200} 200w, ${ImageMiss9Webp400} 400w,  ${ImageMiss9Webp800} 800w`,
    imageJpg: `${ImageMiss9Jpg200} 200w, ${ImageMiss9Jpg400} 400w,  ${ImageMiss9Jpg800} 800w`,
    bioImages: [],
    QAtexts: [
      {
        question: '',
        answer: 'Моя стихия — вода. Ее уникальность в том, что она существует в трёх агрегатных состояниях. Я могу быть плавной и гибкой, как жидкая вода. Могу быть твёрдой и уверенной, как лёд. А могу стать лёгкой и мечтательной, как пар. Для меня быть как вода — значит не бояться проявлять себя с разных сторон.\n\n«Очарование — это не про идеальную внешность, а про возможность притягивать людей своей энергетикой»\n<i>Денисова Ольга</i>',
      },
    ]
  },
];


import ImagePartner0 from '#~/images/miss/missPartners/PartnerMura.svg';
import ImagePartner1 from '#~/images/miss/missPartners/PartnerErnestovich.png';
import ImagePartner2 from '#~/images/miss/missPartners/PartnerRB.png';
import ImagePartner3 from '#~/images/miss/missPartners/PartnerFit.png';
import ImagePartner4 from '#~/images/miss/missPartners/PartnerS.png';
import ImagePartner5 from '#~/images/miss/missPartners/PartnerBall.png';
import ImagePartner6 from '#~/images/miss/missPartners/PartnerPG.png';
import ImagePartner7 from '#~/images/miss/missPartners/PartnerM.png';
import ImagePartner8 from '#~/images/miss/missPartners/PartnerR.png';
import ImagePartner9 from '#~/images/miss/missPartners/PartnerK.png';
import ImagePartner10 from '#~/images/miss/missPartners/PartnerB.png';
import ImagePartner11 from '#~/images/miss/missPartners/PartnerUMP.png';
import ImagePartner12 from '#~/images/miss/missPartners/PartnerTavrida.png';
import ImagePartner13 from '#~/images/miss/missPartners/PartnerADELFI.png';
import ImagePartner14 from '#~/images/miss/missPartners/PartnerREAL.svg';
import ImagePartner15 from '#~/images/miss/missPartners/PartnerREAL.svg';
import ImagePartner18 from '#~/images/miss/missPartners/PartnerVoshod.svg';
import ImagePartner19 from '#~/images/miss/missPartners/PartnerTavrida.png';

export const partnersList = [
  {
    logo: ImagePartner0,
    title: 'Mura Mura',
    description: '<p>Mura Mura - это воплощение элегантности, где классика гармонично сочетается с современными тенденциями. В наших коллекциях вы можете найти самые элегантные и женственные образы. Собрать базовый лаконичный гардероб на все случаи жизни. Мы делаем акцент на деловой стиль, адаптируем тренды под офисный дресс-код, помогая женщинам чувствовать себя уверенней, выделиться в мире «оверсайза» и подчеркнуть свою уникальность и индивидуальность.</p>',
    href: 'https://t.me/miss_bmstu',
  },
  {
    logo: ImagePartner1,
    title: 'Ernestovich',
    description: '<p>«Ernestovich» — официальный магазин бауманской атрибутики. Он здесь, чтобы создавать для вас стиль поколения и новое видение университетского стиля. Мы остановились на том, что близко и понятно каждому бауманцу. Николай Эрнестович Бауман — человек, именем которого назван наш университет.</p>',
    href: 'https://t.me/miss_bmstu',
  },
  {
    logo: ImagePartner2,
    title: 'Ресницы Брови',
    description: '<p>Студия «Ресницы Брови» поможет почувствовать себя по-настоящему очаровательной. Здесь можно отдохнуть, насладиться приятной атмосферой и довериться опытным мастерам. В студии доступны наращивание и ламинирование ресниц, ламинирование и натуральные техники оформления бровей, макияж и укладки. Специалисты работают на качественных материалах, чтобы результат радовал красотой, аккуратностью и стойкостью.</p>',
    href: 'https://vk.ru/rb.moscowresnizibrovi',
  },
  {
    logo: ImagePartner3,
    title: 'БауманкаФИТ',
    description: '<p>«БауманкаФИТ» — современный фитнес-клуб, где есть всё для комфортных и эффективных тренировок. К услугам посетителей профессиональное оборудование, квалифицированные тренеры, тренажёрный зал, бассейн, зоны для групповых занятий и единоборств, сауны и настольный теннис. Для студентов Университета действуют специальные предложения: 3 месяца — 6590 ₽, годовой абонемент — 10990 ₽.</p>',
    href: 'https://baumankafit.ru',
  },
  {
    logo: ImagePartner4,
    title: 'S Parfum&Cosmetics',
    description: '<p>«S Parfum&Cosmetics» — популярный российско-швейцарский бренд косметики и парфюмерии, который ценят за качество, широкий выбор ароматов и доступные цены. Компания была основана в 1995 году, а с 2014 года активно развивает своё присутствие в России. Сегодня бренд располагает собственной производственной и исследовательской базой и предлагает более 300 женских и мужских ароматов — от люксовых и селективных до эксклюзивных композиций.</p>',
    href: 'https://sparfummoscow.ru',
  },
  {
    logo: ImagePartner5,
    title: 'Ballers Street',
    description: '<p>Развлекательный центр «Ballers Street» — место для яркого отдыха с друзьями и всей семьёй. Здесь можно поиграть в боулинг, бильярд и PS5, провести вечер в караоке или на танцполе, а также выбрать VIP-дорожки для более приватной атмосферы. Центр находится в ТРЦ «Ривьера» на втором этаже и работает по понедельникам–четвергам с 10:00 до 22:00, а по пятницам–воскресеньям — с 10:00 до 00:00.</p>',
    href: 'https://t.me/miss_bmstu',
  },
  {
    logo: ImagePartner6,
    title: 'Pretty Garden',
    description: '<p>Pretty Garden — бренд натуральной косметики, в основе которого забота о коже, экологичный подход и разумное потребление. Продукты бренда помогают выстроить простой и эффективный уход без лишнего: очищение, тонизирование и увлажнение для комфортного ежедневного ритуала красоты. Pretty Garden объединяет натуральные компоненты, продуманные формулы и бережное отношение к себе.</p>',
    href: 'https://vk.com/pretty_garden',
  },
  {
    logo: ImagePartner7,
    title: 'Милан',
    description: '<p>Центр красоты «Милан» — пространство, где красота сочетается с профессионализмом, комфортом и вниманием к деталям. С 2007 года здесь помогают женщинам и мужчинам раскрывать свою индивидуальность, предлагая широкий спектр услуг: косметологию, парикмахерский сервис, маникюр и педикюр. Премиальный подход, современные технологии и сильная команда специалистов делают «Милан» местом, куда хочется возвращаться снова и снова.</p>',
    href: 'https://t.me/miss_bmstu',
  },
  {
    logo: ImagePartner8,
    title: 'Ранчо «Сальвадор»',
    description: '<p>Ранчо «Сальвадор» — место силы в Подмосковье, где любовь к лошадям сочетается с природой, уютом и атмосферой свободы. Гостей ждут школа верховой езды для детей и взрослых, уютное кафе, тематические вечера, программы выходного дня для всей семьи и мероприятия под ключ. Для владельцев лошадей предусмотрены комфортные условия содержания. «Сальвадор» — это пространство, где можно вдохновиться, перезагрузиться и почувствовать особую связь с природой.</p>',
    href: 'https://salvador-life.ru',
  },
  {
    logo: ImagePartner9,
    title: 'Керамист',
    description: '<p>Гончарная студия «Керамист» — творческое пространство с камерной атмосферой и стильным интерьером, где можно отвлечься от повседневной суеты и создать что-то своими руками. Здесь проходят мастер-классы по работе за гончарным кругом, занятия по лепке в разных техниках, романтические свидания и другие творческие мероприятия. «Керамист» — это место, где вдохновение превращается в форму.</p>',
    href: 'https://keramist-studio.ru/',
  },
  {
    logo: ImagePartner10,
    title: 'BotHub',
    description: '<p>BotHub — отечественная платформа-агрегатор нейросетей и партнёр конкурса, благодаря которому активисты получили доступ к передовым ИИ-моделям: ChatGPT, Claude, Gemini и другим. Платформа позволяет пользоваться современными нейросетями без VPN и абонентской платы, работать с текстами, изображениями и видео, анализировать документы и выполнять веб-поиск в одном интерфейсе. Для новых пользователей доступно 300 000 капсов.</p>',
    href: 'https://t.me/miss_bmstu',
  },
  // {
  //   logo: ImagePartner11,
  //   title: 'MA AND MI',
  //   description: '<p>MA AND MI — это сеть из семи пространств в Москве, где можно расслабиться, восстановить силы и уделить время себе. Здесь сочетаются уютная атмосфера, качественный сервис и мастера, которые помогают подчеркнуть индивидуальность и почувствовать себя увереннее. MA AND MI — это место, где забота о себе становится особенным ритуалом.</p>',
  //   href: 'https://t.me/miss_bmstu',
  // },
  {  
    logo: ImagePartner12,
    title: 'Таврида.АРТ',
    description: '<p>«Таврида.АРТ» — это Город молодых творцов и мощное сообщество, объединяющее талантливую молодёжь и профессионалов творческих индустрий. Проекты арт-кластера преображают культурные пространства, развивают регионы и популяризируют современное искусство и креативные профессии.</p><p>«Таврида.АРТ» помогает молодым творцам раскрывать свой потенциал, совершенствовать навыки и становиться частью сильного культурного сообщества.</p><p>В число инициатив входят Летние школы Академии «Меганом», фестиваль «Таврида.АРТ», контент-студия «Юг.Кино», сеть арт-резиденций, музыкальный лейбл и бренд «Арт-Молодость». Всё это — для тех, кто уже сегодня создает будущее культуры!</p>',
    href: 'https://tavrida.art/',
  },
  {
    logo: ImagePartner13,
    title: 'Adelfi Beauty Studio',
    description: '<p>Adelfi Beauty Studio — пространство, где эстетика сочетается с заботой, комфортом и вниманием к деталям. Здесь доступны лазерная эпиляция, косметология, массаж, маникюр и педикюр, а также услуги по оформлению бровей и ресниц. Процедуры выполняют опытные мастера с использованием современных технологий и качественных средств. В студии царит атмосфера уюта и гармонии, поэтому сюда приходят не только за результатом, но и за приятными эмоциями.</p>',
    href: 'https://t.me/miss_bmstu',
  },
  {
    logo: ImagePartner14,
    title: 'Автошкола «Реал»',
    description: '<p>«Реал» — сеть современных автошкол, где обучение вождению становится понятным и комфортным. Учеников сопровождают профессиональные преподаватели и опытные инструкторы на каждом этапе обучения. Осваивать теорию можно в том числе онлайн благодаря собственному приложению. Широкая сеть филиалов в Москве и области позволяет выбрать удобную локацию и получить водительские права в комфортном формате.</p>',
    href: 'https://t.me/miss_bmstu',
  },
  // {
  //   logo: ImagePartner15,
  //   title: 'CITY DIAMONDS',
  //   description: '<p>Ювелирный салон CITY DIAMONDS предлагает украшения, которые помогают подчеркнуть важные моменты жизни. В ассортименте представлены изделия с бриллиантами, а также модели известных мировых брендов, включая Cartier и Tiffany & Co., по более доступным ценам. Здесь можно найти украшение как для особого случая, так и для личной покупки, которая будет радовать долгие годы.</p>',
  //   href: 'https://diamondscity.ru',
  // },
  // {
  //   logo: ImagePartner16,
  //   title: 'Tennis Go School',
  //   description: '<p>Tennis Go School — сеть школ большого тенниса и падела, а также организатор международных теннисных кэмпов. Здесь помогают начать занятия с нуля, улучшить технику и стать частью активного спортивного сообщества. Тренировки подходят для любого уровня подготовки и проходят под руководством опытных тренеров. Записаться на занятия можно на сайте школы.</p>',
  //   href: 'https://t.me/miss_bmstu',
  // },
  // {
  //   logo: ImagePartner17,
  //   title: 'КПлазер',
  //   description: '<p>«КПлазер» — студия лазерной эпиляции, где сочетаются комфорт, стерильность и профессиональный подход. Процедуры проводятся на современном аппарате Lotus, который обеспечивает эффективный и бережный результат без боли и раздражения. Уже после нескольких сеансов кожа становится гладкой надолго. Для студентов действует скидка 50% на первое посещение.</p>',
  //   href: 'https://t.me/miss_bmstu',
  // },
  {
    logo: ImagePartner18,
    title: 'Клуб единоборств «Восход»',
    description: '<p>Клуб единоборств «Восход» — это пространство силы, развития и поддержки. Здесь создана дружелюбная атмосфера, в которой комфортно как новичкам, так и опытным спортсменам. Под руководством профессиональных тренеров можно освоить базовые навыки, повысить уровень подготовки и уверенно двигаться к новым результатам. Клуб объединяет людей, увлечённых спортом и стремящихся к постоянному росту.</p>',
    href: 'https://t.me/miss_bmstu',
  },
  {
    logo: ImagePartner19,
    title: 'Таврида.АРТ',
    description: '<p>«Таврида.АРТ» — арт-кластер и сообщество молодых творцов, которое развивает современную культуру и креативные индустрии в России. Проект помогает молодым людям развивать профессиональные навыки, находить единомышленников и реализовывать собственные идеи. В экосистему «Таврида.АРТ» входят образовательные программы Академии «Меганом», фестиваль, арт-резиденции и другие инициативы, направленные на творческий и профессиональный рост.</p>',
    href: 'https://t.me/miss_bmstu',
  },
];

export const phoneNumberToVoting = '5533';
